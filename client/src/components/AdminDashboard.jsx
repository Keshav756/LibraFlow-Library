...
import { RefreshCw, Users, Book, Clock, TrendingUp } from "lucide-react";
  ...
  return (
    <main className="relative flex-1 p-6 pt-28 bg-gray-50 min-h-screen">
      ...
      <section className="space-y-6">
        ...
        {/* Recent activity, top borrowers, genre distribution */}
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
          ...
          {/* Top Borrowers */}
          <article className="bg-white p-5 rounded-lg shadow-md">
            <h4 className="font-semibold mb-4 text-[#3D3E3E]">Top Borrowers</h4>
        ...
        <footer className="bg-white p-4 rounded-lg shadow-md mt-6 flex flex-col md:flex-row justify-between items-center gap-3 text-gray-600 text-sm">
          <div>
            Pro Tip: Keep ISBNs unique and update book quantities when recording
            borrow/return.
          </div>
          <div>
...
const BookCard = ({ title, value, subtitle, icon, colorClass = "from-[#151619] to-[#3D3E3E]" }) => (
  <div className="relative h-48 perspective-1000 group cursor-pointer overflow-hidden">
    <div className={`absolute inset-0 rounded-lg shadow-xl transform transition-all duration-500 group-hover:rotate-y-[-25deg] group-hover:scale-105 overflow-hidden`}>
      <div className={`absolute left-0 top-0 bottom-0 w-8 bg-gradient-to-r ${colorClass} transform-style-3d shadow-inner z-10 overflow-hidden`}>
        <div className="absolute inset-0 bg-black bg-opacity-20 overflow-hidden"></div>
      </div>
      <div className={`absolute left-8 right-0 top-0 bottom-0 bg-gradient-to-br ${colorClass} p-6 flex flex-col justify-between transform-style-3d overflow-hidden`}>
        <div className="flex justify-between items-start overflow-hidden">
          <div className="overflow-hidden">
            <h3 className="text-lg font-bold text-white mb-1 overflow-hidden">{title}</h3>
            <p className="text-4xl font-extrabold text-white tracking-tight overflow-hidden">{Number.isFinite(value) ? value : 0}</p>
          </div>
